...
import os, flet as ft  # já devem existir no arquivo; mantenha
...
def main(page: ft.Page):
    ...
    def _apply_theme_colors():
        ...
        collapse_btn.icon_color = TXT()
        collapse_btn.style.overlay_color = OVERLAY()

        about_btn.icon_color = TXT()
        about_btn.style.overlay_color = OVERLAY()

        theme_btn.icon = ft.Icons.WB_SUNNY if page.theme_mode == ft.ThemeMode.LIGHT else ft.Icons.DARK_MODE
        theme_btn.icon_color = TXT()
        theme_btn.style.overlay_color = OVERLAY()

        alerts_bell.icon_color = TXT()
        alerts_bell.style.overlay_color = OVERLAY()
    ...
    root = ft.Container(
        ...
        content=ft.Row(
            ...
            controls=[
                ...
                ft.VerticalDivider(width=1, color= DIV()),
    ...
    def _safe_render_inicial():
        try:
            ...
            host.content = ft.Container(
                padding=20,
                content=ft.Text(err, color="#B00020", selectable=True),
            )
            page.update()
